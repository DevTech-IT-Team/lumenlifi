...
const navConfig = [
  ...
  },
  {
    ...
    href: '/insights',
    icon: Cpu,
    hoverClass: 'hover:text-[#2AABDB]',
    iconColor: 'group-hover:text-[#2AABDB]',
    dropdown: [
...
>>>>>>> 3f0864a2b11d6f22933447709a4b5bd81bd6b260
          ...
          {navConfig.map((item) => {
            ...
            return (
              <div key={item.label} className="relative">
                ...
                >
                  <NavIcon className={`w-3.5 h-3.5 transition-colors duration-200 ${isOpen_ ? 'text-[#00C2C7]' : 'text-[#2AABDB]/60'} ${item.iconColor}`} />
        ...
        <button
          ...
          className="lg:hidden p-2 rounded-xl text-[#1A6EBF] hover:bg-[rgba(26,110,191,0.08)] transition-colors"
      ...
      {/* ── Mobile drawer ── */}
      {isOpen && (
        <div className="lg:hidden fixed inset-0 top-[70px] bg-white border-t border-[rgba(26,110,191,0.10)] z-40 px-6 py-6 overflow-y-auto animate-in fade-in duration-200">
                ...
                <div key={item.label} className="border-b border-[rgba(26,110,191,0.08)] pb-2 last:border-0 last:pb-0">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <MobIcon className="w-4 h-4 text-[#00C2C7]" />
                      <span>{item.label}</span>
                    </div>
                    {item.dropdown && (
                      <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${mobileExpanded === item.label ? 'rotate-180 text-[#00C2C7]' : 'text-[#4A6080]'}`} />
                    )}
                  </button>
